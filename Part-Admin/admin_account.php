...
?>
    ...
    <body>
        ...
        <div class="container my-3">
            ...
            <form method="post">
                ...
                <button class="btn btn-primary mx-2 btn-sm " name="AddAccounts">Add Accounts</button>
            </form>
        </div>
        <div class="container">
            <table class="table">
                <tbody>
                    ...
                    <form method="post">
                        <h2>Insert information for Accounts as User:</h2>
                        <hr>
                        <div class="mb-3">
                        ...
                        <button type="submit" class="btn btn-primary" name="submit">Add Accounts</button>
                    </form>
                </div>';
                    } elseif (isset($_POST['List'])) {
                        $_POST['Search'] = false;
                        $_POST['approval'] = false;
                        $_POST['AddAccounts'] = false;
                        echo '<thead class="table-dark">
                    <tr>
                        <th scope="col" class="fw-normal">Sl.</th>
                        <th scope="col" class="fw-normal">Acnt_ID</th>
                ...
                WHERE u.user_type = 'accounts' and approval = '1'";
                        $result = mysqli_query($conn, $select);
                        if ($result) {
                    ...
                    <td class="fw-light">' . $account_id . '</th>
                    ...
                    <td>
                    <button class="btn btn-dark btn-sm"><a href="admin_accounts_details.php?detailUID=' . $id . '" class="text-light">Details</a></button>
                ...
                FROM
                    users u
                WHERE u.user_type = 'accounts' and approval = '0'";
                        $result = mysqli_query($conn, $select);
                        if ($result) {
                            while ($row = mysqli_fetch_assoc($result)) {
                                $sl = $row['sl'];
                    ...
                    <td>
                    <button class="btn btn-success btn-sm"><a href="admin_accounts_details.php?detailUID=' . $id . '" class="text-light">Approve & Details</a></button>
                    <button class="btn btn-danger btn-sm" onclick="confirmDelete(' . $id . ')">Delete</button>
                    </td>
                    </tr>    ';
                            }
                ...
                FROM (
                    SELECT 
                        ...
                        u.user_type
                    FROM users u
                    RIGHT JOIN accounts ac ON u.id = ac.user_id
                    WHERE
                        ac.account_id LIKE '%$search%'
                        OR u.name LIKE '%$search%'
                        OR u.email LIKE '%$search%'
                        OR ac.phone_number LIKE '%$search%'
                        OR ac.gender LIKE '%$search%'
                        OR ac.salary LIKE '%$search%'
                ) AS a
                WHERE a.user_type = 'accounts'";
                        $result = mysqli_query($conn, $select);
                        if ($result) {
                            while ($row = mysqli_fetch_assoc($result)) {
                                $sl = $row['sl'];
    ...
    <script>
        function confirmDelete(uid) {
            var result = confirm("Are you sure you want to delete this Accounts?");
            if (result) {
                window.location.href = 'admin_accounts_delete.php?deleteUID=' + uid;
            }
        }
    </script>
